...
public class TestClient {
         ...
         for (final byte desc : descriptor) {
             switch (desc) {
                 case MT_HEADER:
                     ...
                     break;
                 case MT_BODY:
                     final byte[] bytes = data.pollFirst().getData();
                 ...
                 case MT_BODY_REQUEST_CONTEXT:
                     System.out.println("body request context: "+ new String(data.pollFirst().getData()));
                     break;
                 case MT_BODY_EXCEPTION:
                     System.out.println("body exception: "+ new String(data.pollFirst().getData()));
                     break;
                 default:
                     System.out.println("invalid message type (" + desc + "): "+ new String(data.pollFirst().getData()));
             }
         }
    }
    ...
    public void getLocal(){
       ...
       while (true) {
           ...
           if (data != null && data.length != 0) {
               if (Arrays.equals(data, new byte[] {MT_HEADER, MT_BODY_EXCEPTION})) { // header, exception
                   break;
               }
           }
       }
    ...
    private void get(final String devNmae, final String prop, final String selector) {
    ...
    public static void main(String[] args){
        ...
        new TestClient().subscribeSnoopFromDigitizer();
    }
